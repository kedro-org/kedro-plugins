...
dataset-tests:
	cd kedro-datasets && pytest --cov-config pyproject.toml --numprocesses 4 --dist loadfile --ignore tests/tensorflow --ignore tests/databricks --doctest-modules
...
test-no-spark:
	cd kedro-datasets && pytest --no-cov --ignore tests/spark --ignore tests/databricks --numprocesses 4 --dist loadfile --doctest-modules

test-no-spark-sequential:
	cd kedro-datasets && pytest --no-cov --ignore tests/spark --ignore tests/databricks --doctest-modules

# kedro-datasets/snowflake tests skipped from default scope
test-snowflake-only:
	cd kedro-datasets && pytest --no-cov --numprocesses 1 --dist loadfile -m snowflake --doctest-modules
