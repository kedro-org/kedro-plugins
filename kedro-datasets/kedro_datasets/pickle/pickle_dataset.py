...
class PickleDataset(AbstractVersionedDataset[Any, Any]):
    ...
    **Warning:** Pickle-based deserialization (including ``joblib``, ``dill``,
    ``cloudpickle``, and ``compress_pickle`` backends) can execute arbitrary code
    when loading untrusted files. Only load pickle files from sources you trust.
