...
class MatlabDataset(AbstractVersionedDataset[np.ndarray, np.ndarray]):
    ...
    **Warning:** MAT v5 files can contain pickled Python objects.
    ``scipy.io.loadmat()`` will deserialize these, which can execute arbitrary
    code if the file is untrusted. Only load ``.mat`` files from sources you trust.
