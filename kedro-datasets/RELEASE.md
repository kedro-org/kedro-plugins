...
- Fixed `ibis.TableDataset` raising a `ParserException` when overwriting an existing table on the DuckDB backend, by using `insert(..., overwrite=True)` instead of `create_table(..., overwrite=True)`.
